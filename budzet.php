    ...
    if ($budzet) {
        $stmt = $conn->prepare("
            ...
            SET
                naziv = ?,
                datum_vazenja = ?,
                tekuce_po_posebnom_delu = ?,
                investiciono_po_m2 = ?,
                profesionalni_upravnik_po_posebnom_delu = ?,
                garazno_mesto_mesecno = ?,
                pocetno_stanje_racuna = ?,
                nenaplacena_potrazivanja = ?,
                procenat_naplate_potrazivanja = ?,
                procenat_naplate = ?,
                bankarski_troskovi_mesecno = ?,
                nepredvidjeni_troskovi_godisnje = ?
            WHERE id = ?
        ");
        ...
        $stmt = $conn->prepare("
            INSERT INTO budzeti
            (
                sz_id, godina, naziv, datum_vazenja,
                tekuce_po_posebnom_delu,
                investiciono_po_m2,
                profesionalni_upravnik_po_posebnom_delu,
                garazno_mesto_mesecno,
...
$tekuce_mesecno =
    $sz['broj_posebnih_delova'] * val($budzet, 'tekuce_po_posebnom_delu');

$investiciono_mesecno =
    $sz['povrsina_posebnih_delova'] * val($budzet, 'investiciono_po_m2');

$upravnik_mesecno =
    $sz['broj_posebnih_delova'] * val($budzet, 'profesionalni_upravnik_po_posebnom_delu');

$garaze_mesecno =
    $sz['broj_garaznih_mesta'] * val($budzet, 'garazno_mesto_mesecno');

$ukupno_mesecno =
    $tekuce_mesecno + $investiciono_mesecno + $upravnik_mesecno + $garaze_mesecno;

$ukupno_godisnje =
    $ukupno_mesecno * 12;

$ocekivana_naplata =
    $ukupno_godisnje * (val($budzet, 'procenat_naplate', 100) / 100);

$ocekivana_naplata_potrazivanja =
    val($budzet, 'nenaplacena_potrazivanja') *
    (val($budzet, 'procenat_naplate_potrazivanja') / 100);

$bankarski_godisnje =
    val($budzet, 'bankarski_troskovi_mesecno') * 12;
...
$raspolozivo =
    val($budzet, 'pocetno_stanje_racuna')
    + $ocekivana_naplata
    + $ocekivana_naplata_potrazivanja
    + $dodatni_prilivi
    - $dodatni_odlivi
...
<div class="container">
    ...
    <div style="display:flex;justify-content:space-between;align-items:center;">
        ...
        <?php if($budzet): ?>
            <a class="btn"
               href="budzet_stavke.php?budzet_id=<?= $budzet['id'] ?>">
               Stavke budžeta
            </a>
        <?php endif; ?>
    ...
    <div class="grid-2">
        ...
        <form method="POST">
            ...
            <input type="text" name="naziv"
                   value="<?= $budzet ? htmlspecialchars($budzet['naziv']) : 'Osnovni plan' ?>">

            <label>Datum važenja</label>
            <input type="date" name="datum_vazenja"
                   value="<?= $budzet ? $budzet['datum_vazenja'] : date('Y-01-01') ?>">

            <label>Tekuće održavanje po posebnom delu mesečno</label>
            <input type="number" step="0.01" name="tekuce_po_posebnom_delu"
                   value="<?= val($budzet, 'tekuce_po_posebnom_delu') ?>">

            <label>Investiciono održavanje po m² mesečno</label>
            <input type="number" step="0.01" name="investiciono_po_m2"
                   value="<?= val($budzet, 'investiciono_po_m2') ?>">

            <label>Profesionalni upravnik po posebnom delu mesečno</label>
            <input type="number" step="0.01" name="profesionalni_upravnik_po_posebnom_delu"
                   value="<?= val($budzet, 'profesionalni_upravnik_po_posebnom_delu') ?>">

            <label>Garažno mesto mesečno</label>
            <input type="number" step="0.01" name="garazno_mesto_mesecno"
                   value="<?= val($budzet, 'garazno_mesto_mesecno') ?>">

            <label>Početno stanje računa</label>
            <input type="number" step="0.01" name="pocetno_stanje_racuna"
                   value="<?= val($budzet, 'pocetno_stanje_racuna') ?>">

            <label>Nenaplaćena potraživanja</label>
            <input type="number" step="0.01" name="nenaplacena_potrazivanja"
                   value="<?= val($budzet, 'nenaplacena_potrazivanja') ?>">

            <label>Očekivana naplata potraživanja (%)</label>
            <input type="number" step="0.01" name="procenat_naplate_potrazivanja"
                   value="<?= val($budzet, 'procenat_naplate_potrazivanja') ?>">

            <label>Očekivani procenat naplate tekućih zaduženja</label>
            <input type="number" step="0.01" name="procenat_naplate"
                   value="<?= val($budzet, 'procenat_naplate', 100) ?>">

            <label>Bankarski troškovi mesečno</label>
            <input type="number" step="0.01" name="bankarski_troskovi_mesecno"
                   value="<?= val($budzet, 'bankarski_troskovi_mesecno') ?>">

            <label>Nepredviđeni troškovi godišnje</label>
            <input type="number" step="0.01" name="nepredvidjeni_troskovi_godisnje"
                   value="<?= val($budzet, 'nepredvidjeni_troskovi_godisnje') ?>">
