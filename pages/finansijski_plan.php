...
if ($z && $_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'update_plan') {
    ...
    $mesecPocetka = max(1, min(12, (int)post_value('mesec_pocetka', 1)));
    $napomena = trim(post_value('napomena', ''));
    $stmt = $conn->prepare("UPDATE finansijski_planovi SET tekuce_po_delu=?, upravljanje_po_delu=?, garaza_po_mestu=?, investiciono_po_m2=?, investiciono_garaza_po_m2=?, stepen_naplate=?, nepredvidjeni_proc=?, mesec_pocetka=?, napomena=? WHERE id=?");
    $stmt->bind_param('dddddddisi', $tekuce, $upravljanje, $garaza, $invest, $investGaraza, $naplata, $nepred, $mesecPocetka, $napomena, $plan['id']);
    $stmt->execute();
...
<section class="card finance-summary-card" style="margin-top:18px">
    <div class="toolbar">
        ...
        <div class="actions"><a class="btn btn-light btn-sm" target="_blank" href="index.php?page=izvoz_pdf&tip=finansijski_plan&sz_id=<?= $szId ?>&godina=<?= $godina ?>&auto=1">PDF / štampa</a><a class="btn btn-warning btn-sm" href="index.php?page=finansijski_plan_rebalans&sz_id=<?= $szId ?>&godina=<?= $godina ?>">↻ Rebalans</a><span class="badge">mesečna logika</span></div>
    </div>
...
<section class="card" style="margin-top:18px">
    ...
    <div class="month-grid">
        ...
        <div class="month-card <?= $m['stanje'] >= 0 ? 'month-good' : 'month-bad' ?>">
            <strong><?= e($m['naziv']) ?></strong>
            <span>Priliv: <?= money_rs($m['ocekivani_priliv']) ?></span>
            <span>Odliv: <?= money_rs($m['odliv']) ?></span>
            <span>Mesečni saldo: <b><?= money_rs($m['saldo']) ?></b></span>
            <span><strong>Stanje računa: <?= money_rs($m['stanje']) ?></strong></span>
        </div>
        <?php endforeach; ?>
    </div>
</section>
...
<section class="grid grid-2" style="margin-top:18px">
    <div class="card">
        ...
        <form method="post" class="form-grid compact-form">
            ...
            <div class="field"><label>Mesec početka finansijskog plana</label><select name="mesec_pocetka"><?php for($m=1;$m<=12;$m++): ?><option value="<?= $m ?>" <?= ((int)($plan['mesec_pocetka'] ?? 1)===$m)?'selected':'' ?>><?= e(mesec_naziv($m)) ?></option><?php endfor; ?></select><small class="muted">Prilivi i osnovni odlivi računaju se tek od ovog meseca.</small></div>
...
<section class="card" style="margin-top:18px">
    ...
    <div class="table-wrap"><table><thead><tr><th>Stavka</th><th>Obračun</th><th>Period</th><th>Godišnje</th><th>Akcija</th></tr></thead><tbody>
        ...
        <tr><td><strong><?= e($r['naziv']) ?></strong><div class="muted">osnovna stavka</div></td><td><?= e(stavka_osnov_label($r['osnov'])) ?>: <?= e($r['osnovica']) ?> × <?= money_rs($r['iznos']) ?></td><td><?= e(mesec_kratko((int)($r['mesec_od'] ?? 1))) ?>–DEC</td><td><strong><?= money_rs($r['total']) ?></strong></td><td><span class="muted">menja se u osnovnim stavkama</span></td></tr>
        <?php endforeach; ?>
        ...
        <tr>
            ...
            <td><strong><?= money_rs(stavka_total_od_meseca($s, $metrics, (int)($plan['mesec_pocetka'] ?? 1))) ?></strong></td>
...
<section class="card" style="margin-top:18px">
    <div class="toolbar"><h2>Planirani odlivi</h2><a class="btn btn-primary" href="index.php?page=program&sz_id=<?= $szId ?>&godina=<?= $godina ?>">Otvori Program održavanja</a></div><div class="notice">Sve stavke Programa održavanja prikazuju se ovde. Stavke bez definisane cene ne utiču na saldo. Cena može doći iz ponude, cenovnika ili procene. Kod investicija cash-flow prati plan plaćanja/rate, a ne datum izvođenja radova.</div>
    <div class="table-wrap"><table><thead><tr><th>Stavka</th><th>Obračun</th><th>Period</th><th>Godišnje</th><th>Akcija</th></tr></thead><tbody>
        ...
        <?php endforeach; ?>
        <?php foreach(program_stavke_sa_cenama($conn,$szId,$godina) as $ps):
            $bezCene=!program_stavka_ima_cenu($ps);
            $placanje=program_plan_placanja_status($conn,$ps,null);
            $nacin=$ps['nacin_placanja']??'po_terminu';
            $obracun=$bezCene?['broj_termina'=>0,'ukupno'=>0]:program_godisnji_iznos_stavke($conn,(int)$ps['id'],$godina,(float)$ps['planirani_iznos']);
            $uGodini=0.0;
            if(!$bezCene && $nacin!=='po_terminu') foreach(program_placanja_stavke($conn,(int)$ps['id'],$godina) as $rp) $uGodini+=(float)$rp['iznos'];
        ?>
        <tr style="<?= $bezCene?'background:#fff6f7':(!$placanje['kompletno']?'background:#fffbeb':'') ?>">
            <td><strong><?= e($ps['naziv']) ?></strong><div class="muted">Iz Programa održavanja</div><?php if($bezCene): ?><div style="color:#b42318;font-weight:700">Cena nije definisana — izaberi ponudu, cenovnik ili procenu</div><?php elseif(!$placanje['kompletno']): ?><div style="color:#9a3412;font-weight:700">Plan plaćanja nije kompletan</div><?php endif; ?></td>
            <td>
                <?php if($bezCene): ?>—
                <?php else: ?><strong><?= e($ps['izvor_cene_naziv']??program_izvor_cene_label($ps)) ?></strong><div class="muted"><?= e(program_izvor_cene_label($ps)) ?><?= !empty($ps['izvodjac_naziv'])?' · '.e($ps['izvodjac_naziv']):'' ?></div>
                    <?php if(($ps['izvor_cene']??'')==='cenovnik'): ?><div class="muted"><?= e($ps['kolicina']) ?> × <?= money_rs($ps['jedinicna_cena']) ?><?= !empty($ps['cenovnik_jedinica'])?' / '.e($ps['cenovnik_jedinica']):'' ?></div><?php else: ?><div class="muted">Vrednost <?= money_rs($ps['planirani_iznos']) ?></div><?php endif; ?>
                <?php endif; ?>
            </td>
            <td>
                <?php if($bezCene): ?>Nedefinisano
                <?php elseif($nacin==='po_terminu'): ?><?= (int)$ps['ucestalost_meseci'] === 1 ? 'Mesečno' : 'Na svakih '.(int)$ps['ucestalost_meseci'].' mes.' ?> · <?= e(mesec_kratko(max((int)$ps['najraniji_mesec'], ((int)$godina === (int)date('Y') ? (int)date('n') : 1)))) ?>–<?= e(mesec_kratko((int)$ps['krajnji_mesec'])) ?>
                <?php elseif($nacin==='jednokratno'): ?>Jednokratno · prema datumu plaćanja
                <?php else: ?>Na rate · prema datumima rata<?php endif; ?>
            </td>
            <td><strong><?= $bezCene?'—':money_rs($nacin==='po_terminu'?$obracun['ukupno']:$uGodini) ?></strong><?php if(!$bezCene && $nacin!=='po_terminu'): ?><div class="muted">U <?= (int)$godina ?>. godini</div><?php endif; ?></td>
            <td><a class="btn btn-light btn-sm" href="index.php?page=program_ponude&program_id=<?= (int)$ps['id'] ?>&sz_id=<?= $szId ?>&godina=<?= $godina ?>"><?= $bezCene?'Definiši cenu':'Cena / plaćanje' ?></a></td>
        </tr>
        <?php endforeach; ?>
